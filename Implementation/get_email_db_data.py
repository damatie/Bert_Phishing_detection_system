...
OUTPUT_PATH = 'email_phishing_detection_results.json'
...
def fetch_all_data(conn):
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM email_detection_results")
...
def data_to_json(columns, rows):
    ...
    for row in rows:
        ...
        # Parse JSON strings
        if 'keyword_presence' in row_dict:
            row_dict['keyword_presence'] = json.loads(row_dict['keyword_presence'])
